...
class CRM_Ephaimport_Helper {
  ...
  public function clearStatus() {
    $sql = "update tmp_epha_orgdir set status = NULL";
    CRM_Core_DAO::executeQuery($sql);
    $sql = "update tmp_epha_household set status = NULL";
    CRM_Core_DAO::executeQuery($sql);
    $sql = "update tmp_epha_pers set status = NULL";
    CRM_Core_DAO::executeQuery($sql);
  }
  ...
  public function import($tableName, $methodName) {
    ...
    if ($this->queue->numberOfItems() > 0) {
      ...
      for ($i = 0; $i < $totalQueueRuns; $i++) {
        $task = new CRM_Queue_Task(['CRM_Ephaimport_Helper', $methodName], [$numItemsPerQueueRun]);
  ...
  public static function importCleanedOrgs(CRM_Queue_TaskContext $ctx, $limit) {
    ...
    while ($dao->fetch()) {
      ...
      if ($org['count'] == 0) {
        ...
        $updateSQL = "update tmp_epha_orgdir set status = 'OK' where contact_id = " . $dao->contact_id;
  ...
  public static function importHouseholds(CRM_Queue_TaskContext $ctx, $limit) {
    ...
    while ($dao->fetch()) {
      ...
      if ($org['count'] == 0) {
        ...
        $updateSQL = "update tmp_epha_household set status = 'OK' where external_identifier = " . $dao->external_identifier;
        CRM_Core_DAO::executeQuery($updateSQL);
      }
    }
  ...
  public static function importPersons(CRM_Queue_TaskContext $ctx, $limit) {
    $sql = "
      ...
      from
        tmp_epha_pers p
      left outer JOIN
        tmp_epha_employers empl on empl.contact_id = p.external_identifier
      where 
        ifnull(p.status, '') = ''
      and 
    ...
    while ($dao->fetch()) {
      ...
      if ($org['count'] == 0) {
        ...
        */
        ...
        $updateSQL = "update tmp_epha_pers set status = 'OK' where external_identifier = " . $dao->external_identifier;
        CRM_Core_DAO::executeQuery($updateSQL);
      }
    }
