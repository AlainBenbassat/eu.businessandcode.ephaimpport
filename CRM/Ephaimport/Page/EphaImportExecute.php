...
class CRM_Ephaimport_Page_EphaImportExecute extends CRM_Core_Page {

  public function run() {
    CRM_Utils_System::setTitle(E::ts('Epha Import - Execute'));

    $msg = '';
    $action = CRM_Utils_Array::value('action', $_GET);

    $importHelper = new CRM_Ephaimport_Helper();
    ...
    try {
      ...
      elseif ($action == 'importOrgs') {
        $msg = $importHelper->import('tmp_epha_orgdir', 'importCleanedOrgs');
      }
      elseif ($action == 'importHouseholds') {
        $msg = $importHelper->import('tmp_epha_household', 'importHouseholds');
      }
      elseif ($action == 'importPersons') {
        $msg = $importHelper->import('tmp_epha_pers', 'importPersons');
      }
      else {
